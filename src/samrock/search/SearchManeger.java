...
public final class SearchManeger {
	...
	private final IntConsumer mangaManegerWatcher;
	...
	public SearchManeger() {
		...
		mangaManegerWatcher = code -> {
			...
			if(code == MangaManeger.MOD_MODIFIED_INTERNALLY){
	...
	private void loadData() {
		...
		new Thread(() -> {
			...
			for (int i = 0; i < mangaNames.length; i++) mangaNames[i] = mangaManeger.getManga(i).getName().toLowerCase();

			mangaManeger.databaseQuery("SELECT manga_id, description, categories, read_count, unread_count, status FROM MangaData", rs -> {
				...
				try {
					...
					while(rs.next()){
						int index = mangaManeger.getArrayIndex(rs.getInt("manga_id"));
					...
					Utils.openErrorDialoag(null, "Error while extracting search data from datbase : ",SearchManeger.class,505/*{LINE_NUMBER}*/, e);
					System.exit(0);
				}
			});
