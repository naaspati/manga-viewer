...
class IndexedReferenceList<T> {
	...
	public IndexedReferenceList(int maxSize, Class owner, ReferenceType type) {
		...
		this.type = type;
		this.logger = Utils.getLogger(owner.getSimpleName()+"#"+IndexedReferenceList.class.getSimpleName());
	...
	void set(int index, T t) {
		...
		if(index >= array.length) {
			...
			if(debug)
				logger.debug("array resized: {} -> {}", size, array.length);
		}
