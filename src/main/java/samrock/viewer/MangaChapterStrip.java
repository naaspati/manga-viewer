...
class MangaChapterStrip extends JLabel {
	...
	private static final StringBuilder combiner = new StringBuilder();

	private StringBuilder combiner(String start) {
		combiner.setLength(0);
		return combiner.append(start);
	}
	...
	private void setScaleLabel() {
		String b = StringUtils.doubleToString(scale);
		int index = b.indexOf('.');
		if(index > 0 && b.length() - index > 3) 
			b = b.substring(0, index + 3);

		scaleLabel = combiner("zoom : ").append(b).append("X").toString();
	}
	private void setXLabel() {
		xLabel = combiner("x : ").append((int)this.x*-1).toString();
	}
	private void setYLabel() {
		yLabel = combiner("y : ").append((int)this.y*-1).toString();
	}
