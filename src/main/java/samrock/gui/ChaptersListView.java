...
final class ChaptersListView extends JPanel {
	...
	public ChaptersListView(Changer changer) {
		...
		JLabel sideLabel = new JLabel(new ImageIcon(String.format(sideIconLocFormatString, (int)(Math.random()*sideImageCount))));
		...
		pageNumberBox.setRenderer(new ListCellRenderer<int[]>() {
			...
			public Component getListCellRendererComponent(JList<? extends int[]> list, int[] value, int index,
				...
				JLabel ll = new JLabel(("  "+ String.valueOf(c1.getNumber()) +" - " + String.valueOf(c2.getNumber()) + "  ").replace(".0", ""));
	...
	void changeManga() {
		...
		for (int i = 0; ; i++) {
			if(i*MAX_COUNT >= mangaChapterCount - 1){
				vector.add(new int[]{i*MAX_COUNT , i*MAX_COUNT});
				break;
			}
			if((i+1)*MAX_COUNT >= mangaChapterCount){
				vector.add(new int[]{i*MAX_COUNT, mangaChapterCount - 1});
				break;
			}
			vector.add(new int[]{i*MAX_COUNT, (i+1)*MAX_COUNT - 1});
		}
		pageNumberBox.setModel(new DefaultComboBoxModel<>(vector));
		int selectIndex = 0;
